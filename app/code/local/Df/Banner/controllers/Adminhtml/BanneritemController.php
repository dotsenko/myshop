...
class Df_Banner_Adminhtml_BanneritemController extends Mage_Adminhtml_Controller_Action
{
	...
	protected function _initAction() {
		$this->loadLayout()
			...
			->_addBreadcrumb(df_mage()->adminhtml()->__('Рекламные объявления'), df_mage()->adminhtml()->__('Рекламные объявления'));
	...
	public function editAction() {
		...
		$model  = Mage::getModel('df_banner/banneritem')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('df_banner/adminhtml_banneritem_edit'))
				->_addLeft($this->getLayout()->createBlock('df_banner/adminhtml_banneritem_edit_tabs'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(df_helper()->banner()->__('Banner Item does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			//$model = Mage::getModel('df_banner/banneritem');
			//$model->setData($data)
			...
			try {
				...
				$model->save();
				Mage::getSingleton('adminhtml/session')->addSuccess(df_helper()->banner()->__('Объявление сохранено.'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(df_helper()->banner()->__('Unable to find banner item to save'));
        $this->_redirect('*/*/');
	}
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				...
				Mage::getSingleton('adminhtml/session')->addSuccess(df_mage()->adminhtml()->__('Item was successfully deleted'));
				$this->_redirect('*/*/');
			} catch (Exception $e) {
				Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
				$this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
    ...
    public function massDeleteAction() {
        ...
        if(!is_array($dfBannerIds)) {
			Mage::getSingleton('adminhtml/session')->addError(df_mage()->adminhtml()->__('Please select banner item(s)'));
        } else {
            try {
                foreach ($dfBannerIds as $dfBannerId) {
                    $dfBanner = Mage::getModel('df_banner/banneritem')->load($dfBannerId);
                    $dfBanner->delete();
                }
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($dfBannerIds)) {
            ...
            try {
                foreach ($dfBannerIds as $dfBannerId) {
                    $dfBanner = Mage::getSingleton('df_banner/banneritem')
                        ->load($dfBannerId)
    ...
    public function setOrderAction() {
        ...
        if(!$params) {
			Mage::getSingleton('adminhtml/session')->addError(df_mage()->adminhtml()->__('Please select item(s)'));
        } else {
            try {
    ...
    {
        $fileName   = 'df_banner.csv';
        $content    = $this->getLayout()->createBlock('df_banner/adminhtml_banneritem_grid')
            ->getCsv();
