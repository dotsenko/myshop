...
/**
 * Df cms page observer
 *
 * @category    Df
 * @package     Df_Cms
 *
 */
class Df_Cms_Model_Observer
{
	...
	public function cmsControllerRouterMatchBefore(Varien_Event_Observer $observer)
	{
		if (
			...
			if (!$node->getPageId()) {
				...
				$child = Mage::getModel('df_cms/hierarchy_node');
		...
		if (df_enabled (Df_Core_Feature::CMS_2)) {
			...
			if (df_cfg()->cms()->hierarchy()->isEnabled()) {
				...
				if ($page->dataHasChangedFor('identifier')) {
					Mage::getSingleton('df_cms/hierarchy_node')->updateRewriteUrls($page);
				}
				 ...
				 */
				Mage::getSingleton('df_cms/hierarchy_node')->appendPageToNodes($page, $page->getAppendToNodes());
				...
				foreach ($page->getNodesSortOrder() as $nodeId => $value) {
					Mage::getResourceSingleton('df_cms/hierarchy_node')->updateSortOrder($nodeId, $value);
				}
				...
				if ($nodesData) {
					...
					if (!empty($nodesData)) {
						...
						foreach ($nodesData as $row) {
							...
							if (df_parse_boolean(df_a ($row, 'current_page'))) {

								$currentPageIsOrphan = false;


								/** @var int $sortOrder */
								$sortOrder = intval (df_a ($row, 'sort_order'));

								/** @var int $parentNodeId */
								$parentNodeId = intval (df_a ($row, 'parent_node_id'));
								...
								if ($currentPageIsRoot) {
									...
									if ($page->getId()) {
										...
										/** @var Df_Cms_Model_Mysql4_Hierarchy_Node_Collection $currentPageRootNodes */
										$currentPageRootNodes =
											Mage::getResourceModel ('df_cms/hierarchy_node_collection')
										;
										...
										if (0 === $currentPageRootNodes->count()) {
											...
											$node =
												df_model (
													Df_Cms_Model_Hierarchy_Node::getNameInMagentoFormat()
													,
													array (
														'page_id' => $page->getId()
														,
							...
							/** @var Df_Cms_Model_Mysql4_Hierarchy_Node $resourceNode  */
						$resourceNode =
						...
						df_assert ($resourceNode instanceof Df_Cms_Model_Mysql4_Hierarchy_Node);
						...
						if ($currentPageIsOrphan && $page->getId()) {
							 ...
							 */
							$resourceNode->deleteNodesByPageId ($page->getId());

						}


						if (!$currentPageIsRoot) {
							$resourceNode->deleteRootNodesByPageId ($page->getId());
						}
    ...
    public function modifyPageStatuses(Varien_Event_Observer $observer)
    {
		if (
			...
			$statuses->setData(Mage_Cms_Model_Page::STATUS_ENABLED, df_helper()->cms()->__('Published'));
			$statuses->setData(Mage_Cms_Model_Page::STATUS_DISABLED, df_helper()->cms()->__('Unpublished'));
		}
			...
			if (df_cfg()->cms()->versioning()->isEnabled()) {
				...
				if ($page) {
					...
					$baseFieldset->addField('under_version_control', 'select', array(
						'label'     => df_helper()->cms()->__('Under Version Control'),
						'title'     => df_helper()->cms()->__('Under Version Control'),
					...
					if ($page->getPublishedRevisionId() && $page->getUnderVersionControl()) {
						...
						if ($revision->getId()) {
							...
							$page->setPublishedRevisionLink(
								df_helper()->cms()->__('%s; rev #%s', $versionLabel, $revisionNumber));

							$baseFieldset->addField('published_revision_link', 'link', array(
									'label' => df_helper()->cms()->__('Currently Published Revision'),
				...
				if (!$revisionAvailable && $page->getId() && $page->getUnderVersionControl()) {
					...
					$page->setPublishedRevisionStatus(df_helper()->cms()->__('Published Revision Unavailable'));
				}
			}
     ...
     * @return Df_Logging_Model_Event|false
     */
    public function postDispatchCmsHierachyView($config, $eventModel)
    {

		/** @var Df_Logging_Model_Event|false $result  */
		$result =
				(
						df_enabled (Df_Core_Feature::CMS_2)
					&&
			...
			?
				$eventModel->setInfo(df_helper()->cms()->__('Tree Viewed'))
			:
				false
		;
		...
		if (false !== $result) {
			df_assert ($result instanceof Df_Logging_Model_Event);
		}
