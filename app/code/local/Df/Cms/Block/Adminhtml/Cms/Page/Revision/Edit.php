...
class Df_Cms_Block_Adminhtml_Cms_Page_Revision_Edit extends Mage_Adminhtml_Block_Cms_Page_Edit
{
     ...
     *
     * @return Df_Cms_Block_Adminhtml_Cms_Page_Revision_Edit
     */
    public function __construct()
    {
        ...
        if (!empty($objId) && $config->canCurrentUserDeleteRevision()) {
            $this->_addButton('delete_revision', array(
                ...
                'class'     => 'delete',
                'onclick'   => 'deleteConfirm(\''. df_helper()->cms()->__('Are you sure you want to delete this revision?')
                                .'\', \'' . $this->getDeleteUrl() . '\')',
            ));
        }
        ...
        if ($config->canCurrentUserPublishRevision()) {
            $this->_addButton('publish', array(
                'id'        => 'publish_button',
                'label'     => df_helper()->cms()->__('Publish'),
            ...
            if ($config->canCurrentUserSaveRevision()) {
                $this->_addButton('save_publish', array(
                    ...
                    'label'     => df_helper()->cms()->__('Save and Publish'),
        ...
        if ($config->canCurrentUserSaveRevision()) {
            $this->_updateButton('save', 'label', df_helper()->cms()->__('Save'));
            ...
            $this->_addButton('new_version', array(
                'id'        => 'new_version',
                'label'     => df_helper()->cms()->__('Save in New Version...'),
            ...
            $this->_formScripts[] = "
                function newVersionAction(){
                    var versionName = prompt('" . df_helper()->cms()->__('Specify New Version Name (required)') . "', '')
                    if (versionName == '') {
                        alert('" . df_helper()->cms()->__('You should specify valid name') . "');
                        return false;
    ...
    public function getHeaderText()
    {
        ...
        if ($revisionNumber) {
            return df_helper()->cms()->__("Edit Page '%s' Revision #%s", $title, $this->htmlEscape($revisionNumber));
        } else {
            return df_helper()->cms()->__("Edit Page '%s' New Revision", $title);
        }
    }
