...
class Df_Logging_Model_Handler_Controllers
{
    ...
    public function postDispatchGeneric($config, $eventModel, $processorModel)
    {
        if ($collectedIds = $processorModel->getCollectedIds()) {
            $eventModel->setInfo(
                Mage::helper('df_logging')->implodeValues($collectedIds)
            );
            return true;
        }
        return false;
    ...
    */
    ...
    public function postDispatchConfigSave($config, $eventModel, $processor)
    {
        ...
        $change = Mage::getModel('df_logging/event_changes');
    ...
    public function postDispatchPromoCatalogSaveAndApply($config, $eventModel, $processorModel)
    {
        ...
        if ($request->getParam('auto_apply')) {
            $eventModel->setInfo(Mage::helper('df_logging')->__('%s & applied', $eventModel->getInfo()));
        }
    ...
    public function postDispatchTaxRatesImport($config, $eventModel)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('df_logging')->__('Tax Rates Import'));
    }
    ...
    public function postDispatchProductUpdateAttributes($config, $eventModel, $processor)
    {
        ...
        return $eventModel->setInfo(Mage::helper('df_logging')->__('Attributes Updated'));
    }
    ...
    {
        $request = Mage::app()->getRequest();
        $customersQty = Mage::getModel('df_customersegment/segment')->getResource()
                ->getSegmentCustomersQty($request->getParam('id'));
        return $eventModel->setInfo(
            $request->getParam('id') ?
                Mage::helper('df_customersegment')->__('Matched %d Customers of Segment %s', $customersQty, $request->getParam('id')) :
                '-'
        );
    }
    ...
    public function postDispatchSystemCurrencySave($config, $eventModel, $processor)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('df_logging')->__('Currency Rates Saved'));
    }
    ...
    public function postDispatchSaveCacheSettings($config, $eventModel, $processor)
    {
        ...
        switch ($allCache){
            case 'disable':
                $action = Mage::helper('df_logging')->__('Disable');
                break;
            case 'enable':
                $action = Mage::helper('df_logging')->__('Enable');
                break;
            case 'refresh':
                $action = Mage::helper('df_logging')->__('Refresh');
                break;
            default:
                $action = Mage::helper('df_logging')->__('No change');
                break;
        }
        $info = Mage::helper('df_logging')->__('All cache %s. ', $action);
        if ($catalogAction) {
            $info .= $catalogAction;
        }
    ...
    public function postDispatchTaxRatesExport($config, $eventModel)
    {
        ...
        return $eventModel->setIsSuccess($success)->setInfo(Mage::helper('df_logging')->__('Tax Rates Export'));
    }
}
