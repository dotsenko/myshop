...
class Df_Tweaks_Model_Handler_AdjustPoll extends Df_Core_Model_Handler {
	...
	public function handle () {
		...
		if (
				df_cfg()->tweaks()->poll()->getRemoveFromAll()
			||
				(
						df_cfg()->tweaks()->poll()->getRemoveFromFrontpage ()
					&&
						df_handle_presents(Df_Cms_Const::LAYOUT_HANDLE__INDEX_INDEX)
				)
			||
				(
						df_cfg()->tweaks()->poll()->getRemoveFromCatalogProductList ()
					&&
						df_handle_presents(Df_Catalog_Const::LAYOUT_HANDLE__CATEGORY_VIEW)
				)
			||
				(
						df_cfg()->tweaks()->poll()->getRemoveFromCatalogProductView ()
					&&
	...
	public static function getClass () {
		return 'Df_Tweaks_Model_Handler_AdjustPoll';
	}
