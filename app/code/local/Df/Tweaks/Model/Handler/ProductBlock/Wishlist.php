...
class Df_Tweaks_Model_Handler_ProductBlock_Wishlist extends Df_Core_Model_Handler {
	...
	public function handle () {
		...
		if (
				df_cfg()->tweaks()->wishlist()->getRemoveFromAll()
			||
				(
						df_cfg()->tweaks()->wishlist()->getRemoveFromFrontpage ()
					&&
						df_handle_presents(Df_Cms_Const::LAYOUT_HANDLE__INDEX_INDEX)
				)
			||
				(
						df_cfg()->tweaks()->wishlist()->getRemoveFromCatalogProductList ()
					&&
						df_handle_presents(Df_Catalog_Const::LAYOUT_HANDLE__CATEGORY_VIEW)
				)
			||
				(
						df_cfg()->tweaks()->wishlist()->getRemoveFromCatalogProductView ()
					&&
	...
	public static function getClass () {
		return 'Df_Tweaks_Model_Handler_ProductBlock_Wishlist';
	}
