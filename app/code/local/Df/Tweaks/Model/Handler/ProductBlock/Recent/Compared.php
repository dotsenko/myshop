...
class Df_Tweaks_Model_Handler_ProductBlock_Recent_Compared extends Df_Core_Model_Handler {
	...
	public function handle () {
		...
		if (
				df_cfg()->tweaks()->recentlyComparedProducts()->getRemoveFromAll()
			||
				(
						df_cfg()->tweaks()->recentlyComparedProducts()->getRemoveFromFrontpage()
					&&
						df_handle_presents(Df_Cms_Const::LAYOUT_HANDLE__INDEX_INDEX)
				)
			||
				(
						df_cfg()->tweaks()->recentlyComparedProducts()->getRemoveFromCatalogProductList ()
					&&
						df_handle_presents(Df_Catalog_Const::LAYOUT_HANDLE__CATEGORY_VIEW)
				)
			||
				(
						df_cfg()->tweaks()->recentlyComparedProducts()->getRemoveFromCatalogProductView ()
					&&
						df_handle_presents(Df_Catalog_Const::LAYOUT_HANDLE__PRODUCT_VIEW)
				)
			||
				(
						df_cfg()->tweaks()->recentlyComparedProducts()->getRemoveFromAccount()
					&&
						df_handle_presents(Df_Customer_Const::LAYOUT_HANDLE_ACCOUNT)
				)
		) {
			df()->layout()->removeBlock ('catalog.compare.sidebar');
		}
