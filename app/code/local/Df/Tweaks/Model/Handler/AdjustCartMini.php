...
class Df_Tweaks_Model_Handler_AdjustCartMini extends Df_Core_Model_Handler {
	...
	public function handle () {
		...
		if (
				df_cfg()->tweaks()->cart()->getRemoveFromAll ()
			||
				(
						df_cfg()->tweaks()->cart()->getRemoveFromFrontpage ()
					&&
						df_handle_presents(Df_Cms_Const::LAYOUT_HANDLE__INDEX_INDEX)
				)
			||
				(
						df_cfg()->tweaks()->cart()->getRemoveFromCatalogProductList ()
					&&
						df_handle_presents(Df_Catalog_Const::LAYOUT_HANDLE__CATEGORY_VIEW)
				)
			||
				(
						df_cfg()->tweaks()->cart()->getRemoveFromCatalogProductView ()
					&&
						df_handle_presents(Df_Catalog_Const::LAYOUT_HANDLE__PRODUCT_VIEW)
				)
			||
				(
						df_cfg()->tweaks()->cart ()->getRemoveFromAccount()
					&&
						df_handle_presents(Df_Customer_Const::LAYOUT_HANDLE_ACCOUNT)
				)
		) {
			df()->layout()->removeBlock ('cart_sidebar');
		}
