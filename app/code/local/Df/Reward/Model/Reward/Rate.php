 ...
 */
class Df_Reward_Model_Reward_Rate extends Mage_Core_Model_Abstract
{
    ...
    public static function getRateText($direction, $points, $amount, $currencyCode = null)
    {
        switch ($direction) {
            case self::RATE_EXCHANGE_DIRECTION_TO_CURRENCY:
                return df_helper()->reward()->formatRateToCurrency($points, $amount, $currencyCode);
            case self::RATE_EXCHANGE_DIRECTION_TO_POINTS:
                return df_helper()->reward()->formatRateToPoints($points, $amount, $currencyCode);
        }
    }
    ...
    {
        $optArray = array(
            self::RATE_EXCHANGE_DIRECTION_TO_CURRENCY => df_helper()->reward()->__('Points to Currency'),
            self::RATE_EXCHANGE_DIRECTION_TO_POINTS => df_helper()->reward()->__('Currency to Points')
        );
        return $optArray;
    }
