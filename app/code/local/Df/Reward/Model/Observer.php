...
class Df_Reward_Model_Observer
{
    ...
    public function saveRewardNotifications($observer)
    {
        ...
        $data = $request->getPost('reward');
        $subscribeByDefault = df_helper()->reward()->getNotificationConfig('subscribe_by_default');
    ...
    public function orderCompleted($observer)
    {
        ...
        if ($order->getCustomerIsGuest()
            || !df_helper()->reward()->isEnabledOnFront($order->getStore()->getWebsiteId()))
        {
            return $this;
        }
        if ($order->getCustomerId() && ((float)$order->getBaseTotalPaid() > 0)
            ...
            if ($reward->getPointsDelta()) {
                $order->addStatusHistoryComment(
                    df_helper()->reward()->__('Customer earned %s for the order.', df_helper()->reward()->formatReward($reward->getPointsDelta()))
                )->save();
            }
    ...
    public function processBeforeOrderPlace(Varien_Event_Observer $observer)
    {
        ...
        if ($order->getRewardPointsBalance() > 0) {
            ...
            if (($order->getRewardPointsBalance() - $reward->getPointsBalance()) > 0) {
                ...
                Mage::throwException(df_helper()->reward()->__('Not enough Reward Points to complete this Order.'));
            }
        }
        return $this;
    }
    ...
    public function scheduledBalanceExpireNotification()
    {
        ...
        foreach (Mage::app()->getWebsites() as $website) {
            if (!df_helper()->reward()->isEnabledOnFront($website->getId())) {
                continue;
            }
            $inDays = (int)df_helper()->reward()->getNotificationConfig('expiry_day_before');
            if (!$inDays) {
                continue;
            }
            ...
            foreach ($collection as $item) {
                Mage::getSingleton('df_reward/reward')->sendBalanceWarningNotification($item);
            }
        }
    ...
    public function scheduledPointsExpiration()
    {
        ...
        foreach (Mage::app()->getWebsites() as $website) {
            ...
            $expiryType = df_helper()->reward()->getGeneralConfig('expiry_calculation', $website->getId());
    ...
    public function prepareCustomerOrphanPoints(Varien_Event_Observer $observer)
    {
        ...
        Mage::getModel('df_reward/reward')->prepareOrphanPoints($website->getId(), $website->getBaseCurrencyCode());
        return $this;
    }
        ...
        if (df_helper()->reward()->isEnabled()) {
			...
			if (!is_null ($fieldset)) {
				$fieldset->addField('reward_points_delta', 'text', array(
					...
					'label' => df_helper()->reward()->__('Add Reward Points'),
					'title' => df_helper()->reward()->__('Add Reward Points')
    ...
    public function applyRewardSalesrulePoints(Varien_Event_Observer $observer)
    {
        ...
        if (!df_helper()->reward()->isEnabledOnFront($order->getStore()->getWebsiteId())) {
            return $this;
        }
        if ($order->getCustomerId() && !$order->canInvoice() && $order->getRewardSalesrulePoints()) {
            ...
            if ($reward->getPointsDelta()) {
                $order->addStatusHistoryComment(
                    df_helper()->reward()->__('Customer earned promotion extra %s.', df_helper()->reward()->formatReward($reward->getPointsDelta()))
                )->save();
            }
        }
        return $this;
    ...
    public function checkRates(Varien_Event_Observer $observer)
    {
        ...
        df_helper()->reward()->setHasRates($hasRates);
