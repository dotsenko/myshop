...
{
    /**
     * @var Df_CustomerBalance_Model_Mysql4_Balance_Collection
     */
    ...
    protected function _prepareColumns()
    {
        $this->addColumn('updated_at', array(
            'header'    => df_helper()->customer()->balance()->__('Date'),
            'index'     => 'updated_at',
            'type'      => 'datetime',
            'filter'    => false,
            'width'     => 200,
        ...
        $this->addColumn('balance_action', array(
            'header'    => df_helper()->customer()->balance()->__('Action'),
            'width'     => 70,
            'index'     => 'action',
            'sortable'  => false,
            'type'      => 'options',
            'options'   => Mage::getSingleton('df_customerbalance/balance_history')->getActionNamesArray(),
        ));

        $this->addColumn('balance_delta', array(
            'header'    => df_helper()->customer()->balance()->__('Balance Change'),
            'width'     => 50,
            'index'     => 'balance_delta',
            'type'      => 'price',
            'sortable'  => false,
        ...
        $this->addColumn('is_customer_notified', array(
            'header'    => df_helper()->customer()->balance()->__('Customer notified?'),
            'index'     => 'is_customer_notified',
            'type'      => 'options',
            'options'   => array(
                '1' => df_helper()->customer()->balance()->__('Notified'),
                '0' => df_helper()->customer()->balance()->__('No'),
            ),
            'sortable'  => false,
            'filter'    => false,
            'width'     => 75,
        ...
        $this->addColumn('additional_info', array(
            'header'    => df_helper()->customer()->balance()->__('Additional information'),
