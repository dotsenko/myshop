...
class Df_CustomerBalance_Block_Adminhtml_Customer_Edit_Tab_Customerbalance_Balance extends Mage_Adminhtml_Block_Template
{
    /**
    ...
    public function getDeleteOrphanBalancesButton()
    {
        ...
        $balance = Mage::getModel('df_customerbalance/balance');
        if ($balance->getOrphanBalancesCount($customer->getId()) > 0) {
            return $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                'label'     => df_helper()->customer()->balance()->__('Delete Orphan Balances'),
