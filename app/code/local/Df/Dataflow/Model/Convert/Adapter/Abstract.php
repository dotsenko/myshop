...
abstract class Df_Dataflow_Model_Convert_Adapter_Abstract
    ...
    public function parse() {
		...
		try {
			...
			foreach ($this->getImportIds() as $importId) {
				...
				df_assert (
					$importId == $this->getBatchImportModel()->getId ()
					,
					sprintf (
						'Отсутствует пакет данных №%d.'
						,
						$importId
					)
				)
		...
		try {
			...
			$this
				->saveRowInternal (
					$rowAsArray
					,
					df_mage()->adminhtml()->session()->getData (Df_Dataflow_Const::PARAM_COUNTER)
				)
			;
	...
	private function getImportIds () {
		...
		if (!isset ($this->_importIds)) {

			/** @var array $result  */
			$result =
				$this->getBatchImportModel()->getIdCollection()
			;


			df_assert_array ($result);

			$this->_importIds = $result;

		}


		df_result_array ($this->_importIds);
	...
	*/
	 ...
	 */
	private function getBatchImportModel () {

		if (!isset ($this->_batchImportModel)) {

			/** @var Mage_Dataflow_Model_Batch_Import $result  */
			$result =
				df_mage()->dataflow()->batch()->getBatchImportModel()
			;

			df_assert ($result instanceof Mage_Dataflow_Model_Batch_Import);

			$this->_batchImportModel = $result;

		}


		df_assert ($this->_batchImportModel instanceof Mage_Dataflow_Model_Batch_Import);

		return $this->_batchImportModel;
