...
class Df_PromoGift_Model_Handler_SalesRule_Validator_Process_LimitMaxUsagesPerQuote
	 ...
	 * не только для правил с подарками (на которые скидка 100%),
	 * но и для других правил с процентной скидкой.
	 *
	 ...
	 *
	 * Однако, область действия данного модуля — только товары в подарок,
	 * поэтому не пользуемся возможностью расширения сферы данной обработки на другие типы правил,
	 * и обрабатываем только правила с товарами в подарок.
	 *
	 * @return void
	 */
	protected function handlePromoGiftingRule () {
		...
		if (0 < $maxUsagesPerQuote) {
			 ...
			 */
			$timesUsed =
				$this->getRule ()->getData (self::TIMES_USED);
			;
			/** @var int $timesUsed */
			...
			if ($timesUsed > $maxUsagesPerQuote) {
				...
				$this->getCurrentQuoteItem()
					->unsetData (self::DISCOUNT_PERCENT)
				;
	...
	public static function getClass () {
		return 'Df_PromoGift_Model_Handler_SalesRule_Validator_Process_LimitMaxUsagesPerQuote';
	}
