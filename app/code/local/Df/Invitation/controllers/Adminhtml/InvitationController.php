...
class Df_Invitation_Adminhtml_InvitationController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _initInvitation()
    {
        ...
        $invitation = Mage::getModel('df_invitation/invitation')->load($this->getRequest()->getParam('id'));
        if (!$invitation->getId()) {
            Mage::throwException(df_helper()->invitation()->__('Invitation not found.'));
        }
    ...
    public function saveAction()
    {
        try {
            ...
            if (empty($emails)) {
                Mage::throwException(df_helper()->invitation()->__('Specify at least one email.'));
            }
            ...
            foreach ($emails as $key => $email) {
                try {
                    $invitation = Mage::getModel('df_invitation/invitation')->setData(array(
                        'email'    => $email,
                        'store_id' => $storeId,
            ...
            if ($sentCount) {
                $this->_getSession()->addSuccess(df_helper()->invitation()->__('%d invitation(s) were sent.', $sentCount));
            }
            if ($failedCount) {
                $this->_getSession()->addError(df_helper()->invitation()->__('Failed to send %1$d of %2$d invitation(s).', $failedCount, count($emails)));
            }
            if ($customerExistsCount) {
                $this->_getSession()->addNotice(df_helper()->invitation()->__('%d invitation(s) were not sent, because customer accounts already exist for specified email addresses.', $customerExistsCount));
            }
    ...
    public function saveInvitationAction()
    {
        try {
            ...
            if ($this->getRequest()->isPost()) {
                ...
                $this->_getSession()->addSuccess(df_helper()->invitation()->__('Invitation was successfully saved.'));
            }
        }
        catch (Mage_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
    ...
    public function massResendAction()
    {
        try {
            ...
            if (empty($invitationsPost) || !is_array($invitationsPost)) {
                Mage::throwException(df_helper()->invitation()->__('Please select invitations.'));
            }
            ...
            if (!$found) {
                $this->_getSession()->addError(df_helper()->invitation()->__('No invitations have been resent'));
            }
            if ($sent) {
                $this->_getSession()->addSuccess(df_helper()->invitation()->__('%1$d of %2$d invitations were sent.', $sent, $found));
            }
            if ($failed = ($found - $sent)) {
                $this->_getSession()->addError(df_helper()->invitation()->__('Failed to send %d invitation(s).', $failed));
            }
            if ($customerExists) {
                $this->_getSession()->addNotice(
                    df_helper()->invitation()->__('%d invitation(s) cannot be sent, because customer already exists for their emails. These invitations were discarded.', $customerExists)
                );
            }
        }
        catch (Mage_Core_Exception $e) {
    ...
    public function massCancelAction()
    {
        try {
            ...
            if ($cancelled) {
                $this->_getSession()->addSuccess(df_helper()->invitation()->__('%1$d of %2$d invitations were discarded.', $cancelled, $found));
            }
            if ($failed = ($found - $cancelled)) {
                $this->_getSession()->addNotice(df_helper()->invitation()->__('%d of selected invitation(s) were skipped.', $failed));
            }
        }
        catch (Mage_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
    ...
    protected function _isAllowed()
    {
        return df_helper()->invitation()->config()->isEnabled()
            && Mage::getSingleton('admin/session')->isAllowed('customer/df_invitation');
    }
}
