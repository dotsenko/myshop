 ...
 */
class Df_Invitation_Model_Invitation extends Mage_Core_Model_Abstract
{
    ...
    public function loadByInvitationCode($code)
    {
        $code = explode(':', $code, 2);
        if (count($code) != 2) {
            Mage::throwException(df_helper()->invitation()->__('Invalid invitation code.'));
        }
    ...
    protected function _beforeSave()
    {
        if (!$this->getId()) {
            ...
            if (df_helper()->invitation()->config()->getUseInviterGroup()) {
                if ($inviter) {
                    $this->setGroupId($inviter->getGroupId());
                }
                if (!$this->hasGroupId()) {
                    throw new Mage_Core_Exception(df_helper()->invitation()->__('No customer group id specified.'), self::ERROR_INVALID_DATA);
                }
            }
            else {
            ...
            if (!(int)$this->getStoreId()) {
                throw new Mage_Core_Exception(df_helper()->invitation()->__('Wrong store specified.'), self::ERROR_INVALID_DATA);
            }
            $this->makeSureCustomerNotExists();
        }
        else {
            if ($this->dataHasChangedFor('message') && !$this->canMessageBeUpdated()) {
                throw new Mage_Core_Exception(df_helper()->invitation()->__('Message cannot be updated.', self::ERROR_STATUS));
            }
        }
    ...
    {
        Mage::getModel('df_invitation/invitation_history')
        ...
        $mail
            ...
            ->sendTransactional(
                ...
                $this->getEmail(), null, array(
                    'url'           => df_helper()->invitation()->getInvitationUrl($this),
                    'allow_message' => df_is_admin()
                        || df_helper()->invitation()->config()->isInvitationMessageAllowed(),
    ...
    {
        if (!$this->getId()) {
            Mage::throwException(df_helper()->invitation()->__('Impossible to generate encrypted code.'));
        }
    ...
    {
        if (!$this->getId()) {
            throw new Mage_Core_Exception(df_helper()->invitation()->__('Invitation has no ID.'), self::ERROR_INVALID_DATA);
        }
        if ($this->getStatus() !== self::STATUS_NEW) {
            throw new Mage_Core_Exception(
                df_helper()->invitation()->__('Invitation with status "%s" cannot be sent.', $this->getStatus()), self::ERROR_STATUS
            );
        }
        if (!$this->getEmail() || !Zend_Validate::is($this->getEmail(), 'EmailAddress')) {
            throw new Mage_Core_Exception(df_helper()->invitation()->__('Invalid or empty invitation email.'), self::ERROR_INVALID_DATA);
        }
        $this->makeSureCustomerNotExists();
    }
    ...
    public function makeSureCustomerNotExists($email = null, $websiteId = null)
    {
        ...
        if (!$websiteId) {
            throw new Mage_Core_Exception(df_helper()->invitation()->__('Unable to determine proper website.'), self::ERROR_INVALID_DATA);
        }
        if (null === $email) {
            $email = $this->getEmail();
        }
        if (!$email) {
            throw new Mage_Core_Exception(df_helper()->invitation()->__('Email is not specified.'), self::ERROR_INVALID_DATA);
        }
        ...
        throw new Mage_Core_Exception(
            df_helper()->invitation()->__('Customer with email "%s" already exists.', $email), self::ERROR_CUSTOMER_EXISTS
        );
    }
    ...
    {
        $messageInvalid = df_helper()->invitation()->__('This invitation is not valid.');
        if (!$this->getId()) {
    ...
    public function validate()
    {
        ...
        if (!Zend_Validate::is($this->getEmail(), 'EmailAddress')) {
            $errors[] = df_helper()->invitation()->__("Invalid invitation email.");
        }
